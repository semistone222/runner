...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
    ...
    public string NameServerAddress { get { return GetNameServerAddress(); } }
    ...
    protected internal int FriendListAge
    {
        get
        {
            return (isFetchingFriendList || friendListTimestamp == 0) ? 0 : Environment.TickCount - friendListTimestamp;
        }
    }
    ...
    public int mMasterClientId
    {
        get
        {
            ...
            return (CurrentRoom == null) ? 0 : CurrentRoom.MasterClientId;
        }
...
#endif
        ConnectionProtocol currentProtocol = TransportProtocol;
    ...
    public bool Connect(string serverAddress, ServerConnection type)
    {
        ...
        bool connecting = base.Connect(serverAddress, "", TokenForInit);
    ...
    {
        ConnectionProtocol protocolOverride = TransportProtocol;
        ...
        if (socketTcp != null)
        {
            SocketImplementationConfig[ConnectionProtocol.WebSocket] = socketTcp;
            SocketImplementationConfig[ConnectionProtocol.WebSocketSecure] = socketTcp;
        }
...
#pragma warning disable 0162    // the library variant defines if we should use PUN's SocketUdp variant (at all)
        if (NoSocket)
        {
            if (AuthMode != AuthModeOption.AuthOnceWss || serverType != ServerConnection.NameServer)
            {
                if (TransportProtocol != ConnectionProtocol.Udp)
                {
                    Debug.Log("This Photon3Unity3d.dll only allows UDP. TransportProtocol was: " + TransportProtocol + ". SocketImplementation: " + SocketImplementation);
                }
...
#endif
            if (SocketImplementationConfig[ConnectionProtocol.Udp] == null)
            {
...
#pragma warning restore 0162
        ...
        if (TransportProtocol == protocolOverride)
        {
            return;
        }
        ...
        if (PhotonNetwork.logLevel >= PhotonLogLevel.ErrorsOnly)
        {
            Debug.LogWarning("Protocol switch from: " + TransportProtocol + " to: " + protocolOverride + ".");
        }

        TransportProtocol = protocolOverride;
    }
    ...
    private bool CallAuthenticate()
    {
        ...
        AuthenticationValues auth = AuthValues ?? new AuthenticationValues() { UserId = PlayerName };

        if (AuthMode == AuthModeOption.Auth)
        {
            return OpAuthenticate(AppId, AppVersion, auth, CloudRegion.ToString(), requestLobbyStatistics);
        }
        else
        {
            return OpAuthenticateOnce(AppId, AppVersion, auth, CloudRegion.ToString(), EncryptionMode, PhotonNetwork.PhotonServerSettings.Protocol);
        }
    }
    ...
    public override bool OpJoinRoom(EnterRoomParams opParams)
    {
        ...
        lastJoinType = (opParams.CreateIfNotExists) ? JoinType.JoinOrCreateRoom : JoinType.JoinRoom;
    ...
    public virtual bool OpLeave()
    {
        ...
        return OpCustom(OperationCode.Leave, null, true, 0);
    }
    ...
    public bool WebRpc(string uriPath, object parameters)
    {
        ...
        return OpCustom(OperationCode.WebRpc, opParameters, true);
    }
}
