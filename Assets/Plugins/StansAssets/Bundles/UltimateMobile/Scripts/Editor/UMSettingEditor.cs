...
public class UMSettingEditor : Editor {
	...
	private void InAppSettings() {
			...
			if(settings.IsInAppSettingsProductsOpen) {
				...
				foreach(UM_InAppProduct p in settings.InAppProducts) {
					...
					if(p.IsOpen) {
						...
						p.PriceTier	 	= (SA.IOSNative.StoreKit.PriceTier) EditorGUILayout.EnumPopup(p.PriceTier);
						EditorGUILayout.EndHorizontal();
						EditorGUILayout.Space();
		...
		SA.Common.Editor.Tools.BlockHeader("API Settings");

		UltimateMobileSettings.Instance.TransactionsHandlingMode = (UM_TransactionsHandlingMode)SA.Common.Editor.Tools.EnumPopup ("Transactions Handling Mode", UltimateMobileSettings.Instance.TransactionsHandlingMode);

		if(UltimateMobileSettings.Instance.TransactionsHandlingMode == UM_TransactionsHandlingMode.Automatic) {
			IOSNativeSettings.Instance.TransactionsHandlingMode = SA.IOSNative.StoreKit.TransactionsHandlingMode.Automatic;
		} else {
			IOSNativeSettings.Instance.TransactionsHandlingMode = SA.IOSNative.StoreKit.TransactionsHandlingMode.Manual;
		}
	...
	private void OtherSettings() {
		...
		settings.PlatformEngine = (UM_PlatformDependencies)SA.Common.Editor.Tools.EnumPopup ("Platform[?]:", settings.PlatformEngine);
